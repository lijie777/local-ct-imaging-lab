...
import type {
  Advanced3dErrorKind,
  Advanced3dSeriesStatus,
} from '../model/advanced3dViewer'


interface SafeAdvanced3dError {
  kind: Advanced3dErrorKind
  message: string
}
...
function safeRequestError(error: unknown): SafeAdvanced3dError {
  if (!(error instanceof DicomApiError)) {
    return {
      kind: 'unknown',
      message: '无法加载高级 3D，请重试或返回轴位查看器',
    }
  }
  ...
  if (error.code === 'series_not_viewable' || error.status === 409) {
    return {
      kind: 'notViewable',
      message: '该序列暂不可用于高级 3D，请返回轴位查看器',
    }
  }
  if (error.code === 'instance_file_missing' || error.status === 410) {
    return {
      kind: 'persistence',
      message: '本机 DICOM 文件缺失，请恢复文件后重试',
    }
  }
...
export function useAdvanced3dSeries(seriesId: string) {
  const [detail, setDetail] = useState<SeriesDetail | null>(null)
  const [imageIds, setImageIds] = useState<string[]>([])
  const [status, setStatus] = useState<Advanced3dSeriesStatus>('idle')
  const [errorKind, setErrorKind] = useState<Advanced3dErrorKind | null>(null)
    ...
    try {
      ...
      if (!nextEligibility.eligible) {
        ...
        setError(notViewable
          ? '该序列暂不可用于高级 3D，请返回轴位查看器'
          : `高级 3D 暂不可用：${nextEligibility.reason ?? '查看条件不足'}`)
        setStatus('error')
        return
      }
